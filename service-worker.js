...
self.addEventListener('fetch',evento=>{
  ...
  evento.respondWith(caches.match(evento.request).then(cached=>cached||fetch(evento.request).then(respuesta=>{const copia=respuesta.clone();caches.open(VERSION).then(cache=>cache.put(evento.request,copia));return respuesta}).catch(()=>caches.match(evento.request))))
});
